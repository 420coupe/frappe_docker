...
def get_sales_details(doctype):
	is_postgres = frappe.db.db_type != "mariadb"
	datediff_posting = (
		"(CURRENT_DATE - max(so.posting_date))"
		if is_postgres
		else "DATEDIFF(CURRENT_DATE, max(so.posting_date))"
	)
	datediff_transaction = (
		"(CURRENT_DATE - max(so.transaction_date))"
		if is_postgres
		else "DATEDIFF(CURRENT_DATE, max(so.transaction_date))"
	)

	cond = f"""sum(so.base_net_total) as total_order_considered,
			max(so.posting_date) as last_order_date,
			{datediff_posting} as days_since_last_order """
	if doctype == "Sales Order":
		cond = f"""sum(CASE WHEN so.status = 'Stopped' THEN
					so.base_net_total * so.per_delivered/100
					ELSE so.base_net_total END) as total_order_considered,
			max(so.transaction_date) as last_order_date,
			{datediff_transaction} as days_since_last_order"""
