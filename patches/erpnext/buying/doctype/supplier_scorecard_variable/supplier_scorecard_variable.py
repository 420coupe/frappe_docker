...
def get_item_workdays(scorecard):
	...
	_dd_end_schedule = (
		"DATEDIFF( %(end_date)s, po_item.schedule_date)"
		if frappe.db.db_type == "mariadb"
		else "(%(end_date)s::date - po_item.schedule_date)"
	)
	total_item_days = frappe.db.sql(
		f"""
			SELECT
				SUM({_dd_end_schedule} * (po_item.qty))
			FROM
...
def get_total_days_late(scorecard):
	...
	supplier = frappe.get_doc("Supplier", scorecard.supplier)
	_dd_pr_schedule = (
		"DATEDIFF(pr.posting_date,po_item.schedule_date)"
		if frappe.db.db_type == "mariadb"
		else "(pr.posting_date - po_item.schedule_date)"
	)
	_dd_end_schedule = (
		"DATEDIFF( %(end_date)s, po_item.schedule_date)"
		if frappe.db.db_type == "mariadb"
	...
	total_delivered_late_days = frappe.db.sql(
		f"""
			SELECT
				SUM({_dd_pr_schedule}* pr_item.qty)
			FROM
				`tabPurchase Order Item` po_item,
				`tabPurchase Receipt Item` pr_item,
				`tabPurchase Order` po,
	...
	total_missed_late_days = frappe.db.sql(
		f"""
			SELECT
				SUM({_dd_end_schedule} * (po_item.qty - po_item.received_qty))
			FROM
				`tabPurchase Order Item` po_item,
				`tabPurchase Order` po
			WHERE
...
def get_rfq_response_days(scorecard):
	...
	supplier = frappe.get_doc("Supplier", scorecard.supplier)
	_dd_sq_rfq = (
		"DATEDIFF(sq.transaction_date, rfq.transaction_date)"
		if frappe.db.db_type == "mariadb"
		else "(sq.transaction_date - rfq.transaction_date)"
	)
