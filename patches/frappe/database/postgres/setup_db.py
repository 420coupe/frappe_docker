...
def setup_database():
	...
	if psql_version := root_conn.sql("SHOW server_version_num", as_dict=True):
		...
		if cint(semver_version_num) > 150000:
			pass  # Supabase: ALTER DATABASE OWNER TO blocked, GRANT ALL PRIVILEGES is sufficient
	root_conn.close()
...
def import_db_from_sql(source_sql=None, verbose=False):
	...
	# Use root credentials so the pooler username (postgres.projectref) is used
	_import_user = getattr(frappe.flags, "root_login", None) or frappe.conf.db_user
	_import_pass = getattr(frappe.flags, "root_password", None) or frappe.conf.db_password
	DbManager(frappe.local.db).restore_database(
		verbose, db_name, source_sql, _import_user, _import_pass
	)
	if verbose:
